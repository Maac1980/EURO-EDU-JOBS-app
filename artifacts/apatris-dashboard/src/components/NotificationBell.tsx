  ...
  return (
    <div ref={ref} className="relative">
      ...
      >
        ...
        {hasAlerts && (
          <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-lime-400 rounded-full border-2 border-background animate-pulse" />
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 w-80 bg-slate-900 border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-slate-800">
            ...
            {hasAlerts && (
              <span className="px-2 py-0.5 bg-lime-500/30 text-lime-300 text-xs font-bold rounded-full border border-lime-400/30">
                {alerts.length}
              </span>
            )}
          </div>
