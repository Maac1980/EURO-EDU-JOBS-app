...
2. Manish provisions Neon role + sets Fly secret on prod + staging
3. Restart machines — `dbReadOnly` now uses real RO pool; RO protection active without further code changes
...
- Manish-side test: connect to Neon via `psql "$DATABASE_URL_READONLY"`, attempt `INSERT INTO workers (name) VALUES ('test')`, expect `ERROR: permission denied for table workers`. Confirmed protection at infrastructure layer.
...
**Step 1 (already complete):** create `eej_readonly` Neon role + connection string. Stored in password manager.

**Step 2 (pending):** set Fly secret on both apps:
```bash
# Production
~/.fly/bin/flyctl secrets set --app eej-jobs-api DATABASE_URL_READONLY='<RO connection string>'

# Staging
~/.fly/bin/flyctl secrets set --app eej-api DATABASE_URL_READONLY='<RO connection string>'
```

Both `secrets set` operations trigger rolling restart. After restart, startup log will switch from the warning to the activation message. RO protection then live.

**Step 3 (verification):** SSH into prod machine, attempt write via dbReadOnly (or psql with RO connection string), confirm `permission denied`. Document outcome in `docs/STATE_CHANGES_LOG.md` per #14 protocol.
...
**Revision history:**
- v1: 2026-05-10 (Day 22). Initial doc + dbReadOnly export landed via Item 3.0 sub-task 1 commit. Fail-soft fallback when `DATABASE_URL_READONLY` unset.
