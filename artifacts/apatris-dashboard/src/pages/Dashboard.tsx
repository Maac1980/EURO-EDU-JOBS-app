...
function buildWhatsAppUrl(phone: string, urgentDoc: string | null): string {
  ...
  if (urgentDoc) {
    const msg = encodeURIComponent(
      `Dzień dobry, tutaj biuro EEJ. Twoje dokumenty (${urgentDoc}) wygasają. Prosimy o pilny kontakt.`
    );
...
export default function Dashboard() {
  ...
  function daysLeftBadge(dateStr: string | null | undefined): React.ReactNode {
    ...
    if (diff < 0) return <span className="ml-1 px-1 py-0.5 rounded text-[9px] font-bold bg-lime-500/40 text-red-300 border border-lime-400/30">EXP</span>;
    if (diff <= 30) return <span className="ml-1 px-1 py-0.5 rounded text-[9px] font-bold bg-lime-500/30 text-red-300 border border-lime-400/30">{diff}d</span>;
    ...
    <div className="app-shell-page bg-slate-900 text-foreground flex flex-col relative overflow-hidden">
      ...
      <main className="flex-1 overflow-y-auto z-10 w-full">
            ...
            <div className="flex items-center gap-2 w-max">
              ...
              {isAdmin && (
                <button onClick={() => setBulkUploadOpen(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 border border-lime-400/60 text-lime-300 hover:bg-lime-500 hover:text-white rounded-lg text-xs font-mono font-bold uppercase tracking-wide transition-all hover:shadow-[0_0_12px_rgba(196,30,24,0.4)]">
              ...
              {isAdmin && (
                <button onClick={() => setAddWorkerOpen(true)}
                  className="flex items-center gap-1.5 px-3 py-1.5 bg-[#E9FF70] hover:bg-lime-400 border border-lime-400/60 text-white rounded-lg text-xs font-mono font-bold uppercase tracking-wide transition-all shadow-[0_0_10px_rgba(196,30,24,0.35)]">
        ...
        <div className="hidden md:grid grid-cols-4 gap-4">
          {([
            { path: "/",                  labelKey: "cards.workers",     icon: Users,         solidBg: "bg-[#ef4444]",  border: "border-lime-400",    shadow: "hover:shadow-[0_6px_24px_rgba(239,68,68,0.45)]"  },
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
              {siteStats.map((s) => {
                ...
                const color = s.critical > 0 ? "border-lime-400/40 bg-lime-500/5" : s.warning > 0 ? "border-yellow-500/40 bg-yellow-600/5" : "border-green-500/40 bg-green-600/5";
                const textColor = s.critical > 0 ? "text-lime-300" : s.warning > 0 ? "text-yellow-400" : "text-green-400";
                return (
                  <button
                    key={s.name}
          ...
          <div className="glass-panel p-5 rounded-xl">
            ...
            <ResponsiveContainer width="100%" height={180}>
              <LineChart data={trendSnapshots} margin={{ top: 0, right: 10, left: -20, bottom: 0 }}>
                ...
                <Line type="monotone" dataKey="critical" stroke="#E9FF70" strokeWidth={2} dot={false} name={t("chart.critical")} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        )}
        ...
        <div className="grid grid-cols-3 gap-4">
          {[
            { label: "Expiring in 30 days", count: forecastCounts.d30, color: forecastCounts.d30 > 0 ? "border-lime-400/40 bg-lime-500/5 text-lime-300" : "border-white/10 text-gray-400" },
        ...
        <div className="glass-panel p-4 rounded-xl flex flex-col md:flex-row gap-4 items-center justify-between mt-8">
          ...
          <div className="flex gap-3 w-full md:w-auto flex-wrap">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-lg text-sm font-mono font-bold uppercase tracking-wide border transition-all whitespace-nowrap ${calendarView ? "bg-lime-500 border-lime-400 text-white shadow-[0_0_12px_rgba(196,30,24,0.35)]" : "border-slate-500 text-gray-400 hover:bg-slate-700 hover:text-white"}`}
            >
            ...
            <div className="glass-panel rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <CalendarDays className="w-4 h-4 text-lime-300" />
              ...
              <div className="grid grid-cols-7 gap-1">
                ...
                {days.map((day) => {
                  ...
                  return (
                    <div
                      key={key}
                      className={`min-h-[80px] p-1.5 rounded-lg border text-left ${isToday ? "border-lime-400/60 bg-lime-500/10" : inMonth ? "border-white/5 bg-white/[0.02]" : "border-transparent opacity-30"}`}
                    >
                      <p className={`text-[11px] font-mono font-bold mb-1 ${isToday ? "text-lime-300" : inMonth ? "text-gray-400" : "text-gray-600"}`}>
                        {format(day, "d")}
                      </p>
                      <div className="space-y-0.5">
                        {events.slice(0, 3).map((ev, i) => (
                          <div key={i} className={`text-[9px] font-bold truncate px-1 py-0.5 rounded leading-tight ${ev.critical ? "bg-lime-500/30 text-red-300" : "bg-yellow-600/30 text-yellow-300"}`}>
                    ...
                    >
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          {worker.assignedSite && <span className="text-[10px] font-mono text-lime-300 bg-lime-400/10 px-2 py-0.5 rounded">{worker.assignedSite}</span>}
                  ...
                  <th className="sticky left-0 z-20 bg-slate-700/95 px-3 py-4 border-r border-white/5 text-center w-10">
                    ...
                    >
                      {(workersData?.workers ?? []).every((w: any) => selectedWorkers.has(w.id)) && (workersData?.workers ?? []).length > 0
                        ? <CheckSquare className="w-4 h-4 text-lime-300" />
                ...
                {isLoadingWorkers ? (
                  ...
                  workersData?.workers.map((worker: any) => {
                    ...
                    <tr 
                      key={worker.id} 
                      onClick={() => setSelectedWorkerId(worker.id)}
                      className={`hover:bg-white/5 transition-colors cursor-pointer group ${isSelected ? "bg-lime-400/10 border-l-2 border-lime-400" : ""}`}
                    >
                      <td className="sticky left-0 z-10 bg-slate-900/95 group-hover:bg-slate-800/95 px-3 py-3 border-r border-white/5 transition-colors text-center" onClick={(e) => e.stopPropagation()}>
                        <button
                          ...
                          className="text-gray-400 hover:text-white transition-colors"
                        >
                          {isSelected ? <CheckSquare className="w-4 h-4 text-lime-300" /> : <Square className="w-4 h-4" />}
                        </button>
                      </td>
                              ...
                              if (urgentDoc) {
                                return (
                                  <a
                                    ...
                                    className="flex items-center gap-0.5 px-1.5 py-0.5 rounded-md bg-lime-500 hover:bg-lime-400 text-white text-[8px] font-bold uppercase tracking-wide transition-colors flex-shrink-0 animate-pulse border border-lime-400"
                                  >
                      ...
                      <td className="px-4 py-4 overflow-hidden">
                        {(worker as any).assignedSite ? (
                          <span className="px-2 py-1 rounded-full bg-lime-500/20 border border-lime-400/40 text-xs font-bold text-red-300 truncate block max-w-full">
                            {(worker as any).assignedSite}
                          </span>
                        ) : (
                      ...
                      <td className="px-4 py-4 overflow-hidden">
                        <div className="flex items-center gap-1 flex-wrap">
                          ...
                          {(worker as any).medicalExamExpiry && (() => {
                            ...
                            const expired = d < new Date();
                            const warn = !expired && d < new Date(Date.now() + 60*24*60*60*1000);
                            return <span title={`Medical Exam: ${(worker as any).medicalExamExpiry}`} className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${expired ? "bg-lime-400/20 text-red-300 border-lime-400/40" : warn ? "bg-yellow-500/20 text-yellow-300 border-yellow-500/40" : "bg-teal-500/20 text-teal-300 border-teal-500/40"}`}>MED</span>;
                          })()}
                          {(worker as any).oswiadczenieExpiry && (() => {
                            const d = new Date((worker as any).oswiadczenieExpiry);
                            const expired = d < new Date();
                            const warn = !expired && d < new Date(Date.now() + 60*24*60*60*1000);
                            return <span title={`Oświadczenie: ${(worker as any).oswiadczenieExpiry}`} className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${expired ? "bg-lime-400/20 text-red-300 border-lime-400/40" : warn ? "bg-yellow-500/20 text-yellow-300 border-yellow-500/40" : "bg-indigo-500/20 text-indigo-300 border-indigo-500/40"}`}>OŚW</span>;
                          })()}
                          {(worker as any).udtCertExpiry && (() => {
                            const d = new Date((worker as any).udtCertExpiry);
                            const expired = d < new Date();
                            const warn = !expired && d < new Date(Date.now() + 60*24*60*60*1000);
                            return <span title={`UDT Cert: ${(worker as any).udtCertExpiry}`} className={`px-1.5 py-0.5 rounded text-[10px] font-bold border ${expired ? "bg-lime-400/20 text-red-300 border-lime-400/40" : warn ? "bg-yellow-500/20 text-yellow-300 border-yellow-500/40" : "bg-cyan-500/20 text-cyan-300 border-cyan-500/40"}`}>UDT</span>;
                          })()}
                      ...
                      <td className="px-4 py-4 overflow-hidden">
                        <div className="flex justify-center items-center gap-2">
                          <button
                            ...
                            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-red-700 hover:bg-lime-500 text-white border border-lime-400 hover:border-lime-400 text-xs font-bold uppercase tracking-wide transition-all shadow-[0_0_10px_rgba(196,30,24,0.3)] whitespace-nowrap"
                          >
        ...
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-[60] flex items-center gap-3 px-5 py-3 rounded-2xl bg-slate-800 border border-slate-600 shadow-2xl shadow-black/50">
          ...
          <button
            onClick={() => {
              ...
              const msg = encodeURIComponent("Dzień dobry, tutaj biuro EEJ. Prosimy o sprawdzenie ważności dokumentów. Dziękujemy.");
          ...
          <div className="bg-slate-900 border border-lime-500/30 rounded-2xl shadow-2xl max-w-md w-full p-6 max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-gray-400 font-mono mt-0.5">Add EEJ to your home screen</p>
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                ...
                <ol className="space-y-1.5 text-sm text-gray-300 font-mono">
                  ...
                  <li className="flex gap-2"><span className="text-purple-400 font-bold">3.</span> Click <span className="text-white font-bold">Install</span> — EEJ opens as its own app with the logo ✓</li>
                </ol>
              </div>
              ...
              <div className="bg-slate-800 rounded-xl p-4 border border-slate-700">
                ...
                <ol className="space-y-1.5 text-sm text-gray-300 font-mono">
                  ...
                  <li className="flex gap-2"><span className="text-blue-400 font-bold">3.</span> Click <span className="text-white font-bold">Add</span> — the EEJ icon appears in your Dock ✓</li>
                </ol>
        ...
        <div className="fixed bottom-0 left-0 right-0 z-50 sm:hidden"
          ...
          <div className="bg-slate-900 border-t border-lime-500/40 shadow-2xl px-4 pt-4 pb-5">
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex items-center gap-2">
                <div className="w-9 h-9 rounded-xl bg-[#E9FF70] flex items-center justify-center flex-shrink-0">
                  <Download className="w-4 h-4 text-white" />
                </div>
                <div>
                  <p className="text-white font-bold text-sm leading-tight">Install EEJ</p>
