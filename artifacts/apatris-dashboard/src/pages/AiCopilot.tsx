      ...
      <div className="flex-1 overflow-y-auto px-6 space-y-4 pb-4">
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
            {msg.role === "assistant" && (
              <div className="w-8 h-8 rounded-lg bg-lime-400/40 flex items-center justify-center shrink-0 mt-0.5">
                <Bot className="w-4 h-4 text-lime-300" />
              </div>
            )}
            <div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
              msg.role === "user"
                ? "bg-lime-400/40 text-white rounded-br-sm"
        ...
        {loading && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-lg bg-lime-400/40 flex items-center justify-center shrink-0">
              <Bot className="w-4 h-4 text-lime-300" />
            </div>
      ...
      <div className="p-4 border-t border-slate-700/50">
        <form onSubmit={(e) => { e.preventDefault(); ask(input); }} className="flex gap-3">
          <input
            ...
            disabled={loading}
            className="flex-1 bg-slate-800 border border-slate-700/50 rounded-xl px-4 py-3 text-sm text-white placeholder:text-slate-500 focus:outline-none focus:border-lime-400/50 disabled:opacity-50"
          />
          <button
            type="submit"
            disabled={!input.trim() || loading}
            className="px-4 py-3 rounded-xl bg-lime-400/60 text-lime-300 font-bold disabled:opacity-30 hover:bg-lime-400/80 transition-colors"
          >
