...
const STATUS_BADGE: Record<string, string> = {
  ...
  "Below Market": "bg-lime-400/50 text-lime-300 border-lime-400/30",
    ...
    <div className="p-4 md:p-6 min-h-screen overflow-y-auto pb-24 bg-background">
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-6">
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
          <span className="text-xs text-slate-400 uppercase tracking-wide">Avg. EEJ Rate</span>
          <p className="text-2xl font-bold text-white mt-1">{avgEEJ} <span className="text-sm text-slate-400">PLN/h</span></p>
        </div>
        <div className="bg-slate-800/60 border border-slate-700/50 rounded-xl p-4">
        ...
        <table className="w-full min-w-[600px]">
          <thead>
            <tr className="border-b border-slate-700/50">
              ...
              <th className="p-3 text-center text-xs text-slate-400 font-mono uppercase tracking-wide">EEJ (PLN/h)</th>
                ...
                <td className="p-3 text-center">
                  <div className="flex items-center justify-center gap-1">
                    {r.difference > 0 ? (
                      ...
                      <ArrowDown className="w-3.5 h-3.5 text-lime-300" />
                    ) : (
                      <Minus className="w-3.5 h-3.5 text-slate-500" />
                    )}
                    <span className={`text-sm font-mono ${r.difference > 0 ? "text-emerald-400" : r.difference < 0 ? "text-lime-300" : "text-slate-400"}`}>
