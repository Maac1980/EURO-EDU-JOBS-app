...
export default function PricingPage() {
  ...
  const handleCheckout = async (planId: string) => {
    if (planId === "enterprise") {
      window.location.href = "mailto:contact@edu-jobs.eu?subject=Enterprise%20Plan%20Inquiry";
      return;
    }
      ...
      <header className="border-b border-slate-800">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-bold tracking-widest uppercase text-white leading-none">EEJ</p>
              ...
              >
                {plan.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                    <span className="bg-lime-500 text-white text-xs font-bold px-4 py-1 rounded-full uppercase tracking-wide">
                      Most Popular
                    </span>
                  </div>
                )}
                ...
                <ul className="space-y-2.5 mb-8">
                  {plan.features.map(f => (
                    <li key={f} className="flex items-start gap-2 text-sm">
                      <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 ${plan.popular ? "text-lime-300" : "text-emerald-400"}`} />
                ...
                <button
                  ...
                  className={`w-full py-3 rounded-xl text-sm font-bold transition-all flex items-center justify-center gap-2 ${
                    plan.popular
                      ? "bg-lime-500 hover:bg-lime-500 text-white shadow-lg shadow-red-900/30"
