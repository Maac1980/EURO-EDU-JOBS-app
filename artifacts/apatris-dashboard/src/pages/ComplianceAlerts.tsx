...
const STATUS_CONFIG: Record<ComplianceStatus, { label: string; bg: string; border: string; text: string; badge: string; icon: React.ReactNode }> = {
  ...
  RED: {
    ...
    text: "text-lime-300",
    badge: "bg-lime-400/50 text-red-300 border-lime-400/50",
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
      <div className="bg-slate-800 border border-slate-600 rounded-2xl w-full max-w-md p-6 shadow-2xl">
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-lime-400 transition-all"
            />
          </div>
          <div>
            ...
            <select
              ...
              className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-2.5 text-white text-sm focus:outline-none focus:border-lime-400 transition-all appearance-none"
            >
        ...
        {error && (
          <p className="mt-3 text-lime-300 text-sm">{error}</p>
        )}
        ...
        <div className="flex gap-3 mt-6">
          ...
          <button
            onClick={handleSave}
            disabled={saving}
            className="flex-1 py-2.5 bg-lime-500 hover:bg-lime-500 disabled:opacity-50 text-white rounded-lg text-sm font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2"
          >
    ...
    <div className="app-shell-page h-screen bg-slate-900 text-white flex flex-col overflow-hidden">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          {criticalCount > 0 && (
            <span className="flex items-center gap-1 px-2 py-0.5 rounded-full bg-lime-500 text-white text-xs font-bold animate-pulse">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-3 py-1.5 bg-lime-500 hover:bg-lime-500 text-white rounded-lg text-xs font-mono font-bold uppercase tracking-wide transition-all"
          >
      ...
      <main className="flex-1 overflow-y-auto p-6 lg:p-8 max-w-7xl mx-auto w-full space-y-6">
        ...
        {instantAlert && (
          <div className={`flex items-start gap-4 p-4 rounded-xl border ${
            instantAlert.level === "YELLOW"
              ...
              : "bg-lime-400/30 border-lime-400/50 text-red-300"
          }`}>
            <div className="mt-0.5">
              {instantAlert.level === "YELLOW"
                ? <AlertTriangle className="w-5 h-5 text-yellow-400" />
                : <ShieldAlert className="w-5 h-5 text-lime-300" />}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            { label: "Critical ≤30d", count: summary.red, color: "text-lime-300", bg: "bg-lime-400/20 border-red-700/40", status: "RED" as const },
          ...
          {(["ALL", "RED", "EXPIRED", "YELLOW", "GREEN"] as const).map((s) => (
            <button
              ...
              className={`px-3 py-1.5 rounded-lg text-xs font-mono font-bold uppercase tracking-wider border transition-all ${
                filterStatus === s
                  ? "bg-lime-500 border-red-600 text-white"
        ...
        {/* Error */}
        {error && (
          <div className="p-4 rounded-xl bg-lime-400/30 border border-lime-400/40 text-lime-300 text-sm">
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 text-gray-500 hover:text-lime-300 transition-all"
