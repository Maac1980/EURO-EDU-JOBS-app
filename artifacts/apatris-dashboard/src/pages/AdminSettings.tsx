          ...
          <>
            ...
            {sysStatus && (
              <div className="mb-8 grid grid-cols-1 sm:grid-cols-2 gap-4">
                {/* SMTP Status */}
                <div className={`rounded-xl border p-5 flex items-start gap-4 ${sysStatus.smtp.configured ? "border-green-500/30 bg-green-900/10" : "border-lime-400/30 bg-lime-400/10"}`}>
                  <div className={`mt-0.5 flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center ${sysStatus.smtp.configured ? "bg-green-600/20 text-green-400" : "bg-lime-500/20 text-lime-300"}`}>
                    {sysStatus.smtp.configured ? <Wifi className="w-5 h-5" /> : <WifiOff className="w-5 h-5" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-bold ${sysStatus.smtp.configured ? "text-green-400" : "text-lime-300"}`}>
                    ...
                    <div className="space-y-1">
                      {Object.entries(sysStatus.smtp.fields).map(([key, val]) => (
                        <div key={key} className="flex items-center gap-2">
                          <span className={`w-2 h-2 rounded-full flex-shrink-0 ${val === "set" ? "bg-green-500" : "bg-lime-400"}`} />
                          <span className="text-[10px] font-mono text-gray-400">{key}</span>
                          <span className={`text-[10px] font-bold ml-auto ${val === "set" ? "text-green-400" : "text-lime-300"}`}>{val === "set" ? "✓ set" : "missing"}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className={`rounded-xl border p-5 flex items-start gap-4 ${sysStatus.adminPasswords.allSet ? "border-green-500/30 bg-green-900/10" : "border-yellow-500/30 bg-yellow-900/10"}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1">
                      {[["EEJ_PASS_MANISH", sysStatus.adminPasswords.manish], ["EEJ_PASS_AKSHAY", sysStatus.adminPasswords.akshay]].map(([key, set]) => (
            ...
            {!loading && fetchError && (
              <div className="p-4 rounded-xl bg-lime-400/30 border border-lime-400/40 text-lime-300 text-sm flex items-center gap-3">
                  ...
                  return (
                    <div key={admin.id} className="bg-slate-800/60 border border-slate-700 rounded-2xl p-6 lg:p-8" style={{ boxShadow: "0 4px 24px rgba(0,0,0,0.2)" }}>
                      <div className="flex items-center gap-4 mb-6">
                        <div className="w-14 h-14 rounded-full bg-lime-500/20 border-2 border-red-600/50 flex items-center justify-center flex-shrink-0">
                          <span className="text-lg font-bold text-lime-300 font-mono">{initials}</span>
                        </div>
                        <div>
                          <h3 className="text-lg font-bold text-white">{admin.fullName}</h3>
                          <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full bg-lime-400/40 border border-red-600/40 text-lime-300 text-xs font-mono font-bold uppercase tracking-wider mt-1">
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div className="space-y-2">
                          ...
                          <input type="email" value={state.email} onChange={(e) => updateField(admin.id, "email", e.target.value)} placeholder="email@example.com"
                            className="w-full bg-slate-900 border border-slate-600 rounded-lg px-4 py-3 text-white text-sm font-mono focus:outline-none focus:border-lime-400 focus:ring-1 focus:ring-red-500/40 transition-all placeholder:text-gray-600" />
                        </div>
                        <div className="space-y-2">
                          <label className="flex items-center gap-2 text-xs font-bold text-gray-400 uppercase tracking-widest">
                      ...
                      {state.error && (
                        <div className="mt-4 flex items-center gap-2 text-lime-300 text-sm">
                      ...
                      <div className="mt-6 flex justify-end">
                        <button onClick={() => saveAdmin(admin)} disabled={state.saving}
                          className="flex items-center gap-2 px-6 py-2.5 bg-lime-500 hover:bg-lime-500 disabled:bg-lime-400/50 disabled:cursor-not-allowed text-white font-bold text-sm uppercase tracking-widest rounded-lg transition-all shadow-lg shadow-red-900/30">
                  ...
                  <tbody className="divide-y divide-slate-700">
                    {notifLog.map((e) => (
                      <tr key={e.id} className="hover:bg-slate-800/50 transition-colors">
                        ...
                        <td className="px-4 py-3 text-lime-300 font-mono text-xs">{e.expiryDate ? format(parseISO(e.expiryDate), "MMM d, yyyy") : "—"}</td>
                        <td className="px-4 py-3">
                          <span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${e.status === "EXPIRED" ? "bg-lime-400/50 text-red-300" : "bg-orange-900/50 text-orange-300"}`}>
                            {e.status}
                          </span>
                        </td>
                        <td className="px-4 py-3">
                          {e.sent ? (
                            ...
                            <span className="flex items-center gap-1 text-lime-300 text-xs"><AlertCircle className="w-3.5 h-3.5" /> Failed</span>
                          )}
                        </td>
                      </tr>
                    ))}
        ...
        {activeTab === "site-coordinators" && (
          <>
            ...
            <div className="rounded-xl border border-slate-700 bg-slate-800/50 p-6 mb-8">
              <h3 className="text-sm font-bold uppercase tracking-widest text-gray-300 mb-4 flex items-center gap-2"><Plus className="w-4 h-4 text-lime-300" /> Add New Coordinator</h3>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-lime-400/60 placeholder:text-gray-600"
                  />
                </div>
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-lime-400/60 placeholder:text-gray-600"
                    />
                  </div>
                </div>
                <div>
              ...
              {addCoordError && (
                <p className="text-lime-300 text-xs mt-3 flex items-center gap-1.5"><AlertCircle className="w-3.5 h-3.5" />{addCoordError}</p>
              )}
              {addCoordSuccess && (
              ...
              <button
                ...
                className="mt-4 flex items-center gap-2 px-5 py-2.5 rounded-lg bg-lime-500 hover:bg-lime-400 disabled:opacity-50 text-white text-sm font-bold uppercase tracking-wide transition-colors border border-lime-400"
              >
                      ...
                      return (
                        <React.Fragment key={c.id}>
                          <tr className="hover:bg-slate-800/50 transition-colors">
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-3">
                                <div className="w-8 h-8 rounded-full bg-lime-500/20 border border-lime-400/30 flex items-center justify-center text-lime-300 text-xs font-bold">
                            ...
                            <td className="px-4 py-3">
                              <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-lime-500/20 border border-lime-400/30 text-red-300">{c.assignedSite}</span>
                            </td>
                            <td className="px-4 py-3 text-right">
                              <div className="flex items-center justify-end gap-2">
                                ...
                                <button
                                  ...
                                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-lime-400/30 hover:bg-lime-400/60 border border-lime-400/30 text-lime-300 hover:text-red-300 text-xs font-bold uppercase tracking-wide transition-colors disabled:opacity-50"
                                >
                                ...
                                <div className="flex items-center gap-3 flex-wrap">
                                  ...
                                  <div className="relative flex-1 min-w-[180px]">
                                    <input
                                      ...
                                      className="w-full bg-slate-900 border border-slate-600 text-white rounded-lg px-3 py-2 pr-9 text-sm font-mono focus:outline-none focus:border-lime-400/60 placeholder:text-gray-600"
                                    />
                                    <button
                                      type="button"
                                  ...
                                  <button
                                    ...
                                    className="flex items-center gap-1.5 px-4 py-2 rounded-lg bg-lime-500 hover:bg-lime-400 disabled:opacity-60 text-white text-xs font-bold uppercase tracking-wide transition-colors border border-lime-400 shrink-0"
                                  >
                                  ...
                                  {ps.error && <span className="text-lime-300 text-xs flex items-center gap-1"><AlertCircle className="w-3 h-3" />{ps.error}</span>}
                                </div>
                              </td>
                            </tr>
                          )}
