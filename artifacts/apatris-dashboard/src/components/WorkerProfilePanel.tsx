...
function buildWaUrl(phone: string, urgentDoc?: string | null): string {
  ...
  if (urgentDoc) {
    const msg = encodeURIComponent(`Dzień dobry, tutaj biuro EEJ. Twoje dokumenty (${urgentDoc}) wygasają. Prosimy o pilny kontakt.`);
    ...
    <div ref={wrapRef} className="relative w-full">
      <div className="relative">
        <input
          ...
          className="w-full bg-slate-800 border border-slate-600 text-white rounded-lg pl-3 pr-9 py-2 text-sm font-mono focus:outline-none focus:border-lime-400/60 placeholder:text-gray-600 transition-colors"
        />
      ...
      {open && (
        <div className="absolute z-50 mt-1 w-full rounded-lg bg-slate-800 border border-lime-400/20 shadow-2xl overflow-hidden">
          {isNew && (
            <button type="button" onMouseDown={() => select(query.trim())} className="w-full flex items-center gap-2 px-3 py-2.5 text-sm font-mono text-lime-300 hover:bg-lime-400/10 border-b border-slate-700 transition-colors">
          ...
          {filtered.map((s) => (
            <button key={s} type="button" onMouseDown={() => select(s)} className={`w-full flex items-center gap-2 px-3 py-2.5 text-sm font-mono text-left hover:bg-white/5 transition-colors ${s === value ? "text-lime-300 bg-lime-400/10" : "text-gray-300"}`}>
              <MapPin className="w-3.5 h-3.5 flex-shrink-0 text-gray-500" />
              {s}
              {s === value && <span className="ml-auto text-[10px] text-lime-300 font-bold">CURRENT</span>}
            </button>
          ))}
        </div>
      )}
...
function FRow({ label, value, accent }: { label: string; value?: string | null; accent?: "green" | "red" | "yellow" | "blue" }) {
  const colors = { green: "text-green-400", red: "text-lime-300", yellow: "text-yellow-400", blue: "text-blue-400" };
  return (
...
function DocRow({ label, date }: { label: string; date?: string | null }) {
  ...
  return (
    <div className={`flex items-center justify-between p-3 rounded-lg bg-slate-800 border ${isExpired ? "border-lime-400/40" : isWarning ? "border-yellow-500/40" : "border-slate-700"}`}>
      <span className="text-sm font-medium text-gray-300">{label}</span>
      <span className={`text-sm font-mono font-semibold ${isExpired ? "text-lime-300" : isWarning ? "text-yellow-400" : "text-green-400"}`}>
        {format(d, "MMM d, yyyy")}
      </span>
    </div>
  );
...
function AttachmentCard({ title, filename, url }: { title: string; filename: string; url: string }) {
  return (
    <a href={url} target="_blank" rel="noopener noreferrer" className="relative p-4 rounded-xl bg-slate-800 border border-slate-700 hover:bg-slate-700 hover:border-lime-400/30 transition-all cursor-pointer group flex flex-col items-center justify-center text-center gap-2">
      <div className="w-10 h-10 rounded-full bg-lime-400/10 text-lime-300 flex items-center justify-center group-hover:scale-110 transition-transform">
...
function UploadButton({ workerId, docType, label, accent = "red" }: { workerId: string; docType: "passport" | "contract" | "trc" | "bhp"; label: string; accent?: "red" | "green" | "orange" | "violet" }) {
  ...
  const accentClasses = { red: "border-lime-400/70 hover:bg-lime-400/10 hover:text-white", green: "border-green-500/70 hover:bg-green-500/10 hover:text-green-300", orange: "border-orange-500/70 hover:bg-orange-500/10 hover:text-orange-300", violet: "border-violet-500/70 hover:bg-violet-500/10 hover:text-violet-300" };
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/75 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-slate-900 border border-white/15 rounded-2xl shadow-2xl w-full max-w-xs p-6 flex flex-col items-center gap-4" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="w-full flex items-center justify-between">
          <span className="text-[10px] font-bold uppercase tracking-widest text-lime-300 font-mono">Worker QR</span>
        ...
        <div className="w-full bg-slate-800/60 rounded-xl border border-white/10 p-3 space-y-2">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            {worker.assignedSite && (
              <span className="text-[10px] font-bold text-red-300 bg-lime-500/20 border border-lime-400/30 rounded px-2 py-0.5">{worker.assignedSite}</span>
            )}
          </div>
            ...
            {docs.map(([label, expiry]) => {
              ...
              return (
                <div key={label} className="flex items-center justify-between bg-slate-900/60 rounded px-2 py-1">
                  ...
                  <span className={`text-[10px] font-bold font-mono ${isExpired ? "text-lime-300" : isSoon ? "text-yellow-400" : "text-green-400"}`}>{dl}</span>
                </div>
              );
            })}
          </div>
        ...
        {/* Download button */}
        <button onClick={downloadQR} className="w-full flex items-center justify-center gap-2 py-2 rounded-xl bg-red-700 hover:bg-lime-500 border border-lime-400 text-white text-xs font-bold uppercase tracking-wider transition-colors">
...
function PIPModal({ worker, onClose }: { worker: any; onClose: () => void }) {
  ...
  function docStatus(date: string | null) {
    ...
    if (days < 30) return { label: `${days}d (CRITICAL)`, cls: "text-lime-300 font-bold" };
  ...
  return (
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div className="bg-white text-black rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        {/* Header */}
        <div className="bg-[#E9FF70] text-white p-5 flex items-start justify-between rounded-t-xl">
          <div>
            <p className="text-xs font-mono opacity-70 uppercase tracking-widest">PIP Inspection Card — EEJ Sp. z o.o.</p>
        ...
        <div className="px-5 pb-5 flex gap-3">
          <button onClick={() => window.print()} className="flex-1 flex items-center justify-center gap-2 py-2 bg-[#E9FF70] text-white rounded-lg text-sm font-bold hover:bg-lime-500 transition-colors">
...
export function WorkerProfilePanel({ workerId, initialEditMode = false, onClose, onRenew, onNotify }: WorkerProfilePanelProps) {
  ...
  const handlePrintFinalSettlement = () => {
    ...
    doc.text("EEJ SP. Z O.O.", 105, 20, { align: "center" });
    doc.setFontSize(13);
    ...
    doc.text("EEJ SP. Z O.O. · ul. Chłodna 51, 00-867 Warszawa · NIP: 5252828706 · KRS: 0000849614 · REGON: 386546470", 105, 287, { align: "center" });
  ...
  const inputCls = "w-full bg-slate-800 border border-slate-600 text-white rounded-lg px-3 py-2 text-sm font-mono focus:outline-none focus:border-lime-400/60 placeholder:text-gray-600";
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast({ title: "Welder Records Updated", description: "All fields saved. Compliance status recalculated.", className: "border-lime-400/50 bg-slate-900 text-white [&>div]:text-lime-300" });
      setIsEditing(false);
    } catch (err) {
          ...
          <div className="flex flex-col h-full min-h-0">
            ...
            <div className="flex-shrink-0 p-6 border-b border-white/10 relative overflow-hidden bg-slate-900">
              <div className="absolute top-0 right-0 p-4 flex items-center gap-2">
                ...
                {!isEditing && (
                  <button onClick={() => setIsEditing(true)} className="flex items-center gap-1.5 px-3 py-1.5 bg-red-700 hover:bg-lime-500 border border-lime-400 rounded-lg transition-colors text-xs font-bold text-white uppercase tracking-wider">
              ...
              <div className="flex items-center gap-4 mt-4">
                <div className="w-16 h-16 rounded-xl bg-lime-500/20 border border-lime-400/30 flex items-center justify-center text-lime-300 text-xl font-bold uppercase">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    ...
                    {(worker as any).assignedSite && (
                      <span className="px-2 py-0.5 rounded-full text-xs font-bold bg-lime-500/20 border border-lime-400/30 text-red-300 flex items-center gap-1">
            ...
            <div className="p-4 space-y-4 flex-1 overflow-y-auto min-h-0">
              ...
              {isEditing && (
                <div className="space-y-3">
                  <p className="text-[10px] font-bold uppercase tracking-widest text-lime-300 flex items-center gap-2">
                  ...
                  <div className="flex gap-2 pt-1">
                    ...
                    <button onClick={handleSave} disabled={saving} className="flex-1 py-2 bg-red-700 hover:bg-lime-500 text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-1.5 shadow-[0_0_15px_rgba(196,30,24,0.35)]">
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-red-700 hover:bg-lime-500 disabled:opacity-50 disabled:cursor-not-allowed text-white rounded-lg text-xs font-bold uppercase tracking-wider transition-all shadow-[0_0_12px_rgba(196,30,24,0.3)]"
                    >
                ...
                <>
                  ...
                  <div className="grid grid-cols-1 gap-3 p-4 rounded-xl bg-slate-800 border border-slate-700">
                    <div className="flex items-center gap-3 text-sm">
                      <Mail className="w-4 h-4 text-lime-300 flex-shrink-0" />
                      <span className="text-gray-300 font-mono">{(worker as any).email || t("panel.noEmail")}</span>
                    </div>
                    <div className="flex items-center gap-2 text-sm flex-wrap">
                      <Phone className="w-4 h-4 text-lime-300 flex-shrink-0" />
                      {(worker as any).phone
                      ...
                      {(worker as any).phone && (
                        <div className="flex items-center gap-1.5 flex-shrink-0 flex-wrap mt-1">
                          ...
                          {(() => {
                            ...
                            return urgentDoc ? (
                              <a href={waUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-lime-500 hover:bg-lime-400 border border-lime-400 text-white text-xs font-bold transition-colors animate-pulse"><AlertTriangle className="w-3.5 h-3.5" />{t("comm.urgentAlert")}</a>
                            ) : (
                    ...
                    {(worker as any).assignedSite && (
                      <div className="flex items-center gap-3 text-sm">
                        <MapPin className="w-4 h-4 text-lime-300 flex-shrink-0" />
                      ...
                      <div className="rounded-xl border border-slate-700 bg-slate-800/50 overflow-hidden">
                        <div className="px-4 py-3 border-b border-slate-700 flex items-center justify-between">
                          <div className="flex items-center gap-2">
                            <CheckSquare className="w-3.5 h-3.5 text-lime-300" />
                            <p className="text-xs font-bold uppercase tracking-widest text-gray-400">Onboarding Checklist</p>
                          </div>
                          <div className="flex items-center gap-2">
                            <div className="w-24 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                              <div className="h-full rounded-full transition-all" style={{ width: `${pct}%`, background: pct === 100 ? "#22c55e" : pct >= 70 ? "#eab308" : "#E9FF70" }} />
                            </div>
                            <span className={`text-xs font-mono font-bold ${pct === 100 ? "text-green-400" : pct >= 70 ? "text-yellow-400" : "text-lime-300"}`}>{done}/{checks.length}</span>
                          </div>
                        </div>
            ...
            {deleteConfirm && isAdmin && (
              <div className="mx-5 mb-3 p-4 rounded-xl bg-red-950/60 border border-lime-400/50">
                <p className="text-sm font-bold text-red-300 mb-1">Permanently delete this worker?</p>
                <p className="text-xs text-lime-300 font-mono mb-3">This cannot be undone. The record will be removed from Airtable.</p>
                <div className="flex gap-2">
                  ...
                  <button onClick={handleDelete} disabled={deleting} className="flex-1 py-1.5 text-xs font-bold bg-red-700 hover:bg-lime-500 text-white rounded-lg transition-colors flex items-center justify-center gap-1.5">
            ...
            <div className="p-5 border-t border-white/10 bg-slate-800/50 flex gap-3">
              {isAdmin && !isEditing && (
                <button
                  ...
                  className="p-2.5 bg-white/5 hover:bg-lime-400/40 text-gray-500 hover:text-lime-300 border border-white/10 hover:border-lime-400/40 rounded-xl transition-all"
                  title="Delete Worker"
                >
              ...
              <button className="flex-1 py-2.5 bg-red-700 hover:bg-lime-500 text-white rounded-xl font-bold uppercase tracking-wider text-sm transition-all shadow-[0_0_15px_rgba(220,38,38,0.3)]" onClick={() => onRenew(worker)}>
                {t("panel.renewDoc")}
              </button>
            </div>
          </div>
